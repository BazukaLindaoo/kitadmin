...
Title = "KiKitk4t Hub ambiente seguro!",
SubTitle = "by Bazuka",
SaveFolder = "KitK4t | redz lib v5.lua"
})
...
local Paragraph = Tab1:AddParagraph({"Test", "não tem funções no painel, apenas no chat. "})
...
local function lagarJogoFireX(fireXPath, maxTeleports)
    if fireXPath then
        ...
        -- Após 2 minutos, parar tudo e clicar na tela com Heartbeat
        task.delay(300, function()
            if toggles.LagFireX then
                ...
                clickConnection = RunService.Heartbeat:Connect(function()
                    ...
                    VirtualUser:ClickButton1(Vector2.new(500, 500)) -- posição genérica
                end)
            end
        end)
    else
